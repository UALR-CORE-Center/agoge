...
if __name__ == "__main__":
    ...
    response = str(input(f"What type of Maintenance do you want to run? ([D]aily, [H]ourly, [Q]uarter-hourly, [C]reate Snapshot, [R]emove Snapshot) "))
    ...
    elif str.upper(response).startswith("C"):
        DailyMaintenance(debug=True)._snapshot_workouts()
    elif str.upper(response).startswith("R"):
        HourlyMaintenance(debug=True)._delete_expired_snapshots()
