...
test('API transformation retains authoritative ownership and image compatibility metadata', () => {
    const shared = transformToAgogeImage({name: 'router', is_shared: true, can_edit_shared: true, source_project: 'shared-resources', architecture: 'ARM64', base_family: 'ubuntu', state_timestamp: '2026-09-12T12:00:00Z'});
    assert.equal(shared.is_shared, true);
    assert.equal(shared.can_edit_shared, true);
...
test('missing or non-boolean administrator capability never enables shared editing', () => {
    for (const can_edit_shared of [undefined, null, false, 'true', 'false', 1]) {
        assert.equal(transformToAgogeImage({is_shared: true, can_edit_shared}).can_edit_shared, false);
    }
});
