...
export const SelectMachineTypeTable: React.FC<Props> = (props) => {
    ...
    const getMType = () => {
        return machineTypes.find(mType => mType.id == props.formField.value);
    }
    ...
    const initialValue = () => {
        if (props.formField && !isInitialized) {
            const mTypeId = props.formField.value;
            const defaultRow = getMType();

            if (apiRef.current && defaultRow != undefined) {
                apiRef.current.setRowSelectionModel([mTypeId]);
        ...
        },
        {
            ...
            renderCell: (params: any) => (
                <Button
                    variant="text"
                    tabIndex={0}
                    aria-label={`Select ${params.value || "machine type name"}`}
                >
                    {params.value || "No machine type name."}
                </Button>
            ),
        },
        {
