...
class LMSBlackboard(LMS):
    def __init__(self, url, api_key, secret_key, course_code, build, env_dict=None):
        super().__init__(url, api_key, course_code, build)
        self.env = CloudEnv(env_dict=env_dict) if env_dict else CloudEnv()
        self.blackboard = BbRest(self.url, self.api_key, self.secret_key)
        self.course = self.blackboard.GetCourse(self.courseId)
        self.class_list = self.blackboard.GetUsers(courseId=self.course.courseId, enrollment_type=['student'])
    ...
    def create_quiz(self, delete_existing_quizzes=True):
        ...
        for question in json.loads(json.dumps(questions)):
            ...
            question_data = {
                ...
                'question_type': question.get('question_type', BlackboardConstants.Questions.Types.SHORT_ANSWER),
