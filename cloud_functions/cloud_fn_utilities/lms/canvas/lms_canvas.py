...
class LMSCanvas(LMS):
    def __init__(self, url, api_key, course_code, build, env_dict=None):
        super().__init__(url=url, api_key=api_key, course_code=course_code, build=build)
        ...
        for student in self.class_list:
            try:
                ...
                if hasattr(student, 'name') and not suppress_logs:
                    self.logger.warning(f"{self.class_name}:{self.course_code} - Email does not exist for {student.name} "
                                        f"and a Workout will not be created for them!")
                elif not suppress_logs:
                    self.logger.warning(f"{self.class_name}:{self.course_code} - Error when trying to enumerate class "
                                        f"list. Student record has no name!")
        return students

    def create_quiz(self, delete_existing_quizzes=True):
        questions = self.build['lms_integration']['questions']
        ...
        quiz_data = {
            ...
            'due_at': self.build['lms_integration']['due_at'],
    ...
    def _store_quiz_identifiers(self, quiz_key: int, question_ids: list):
        self.build['lms_integration']['quiz_key'] = quiz_key
        for i, question in enumerate(self.build['lms_integration']['questions']):
