...
from api.utilities.lms.lms_canvas import LMSCanvas
from api.utilities.lms.lms_blackboard import LMSBlackboard
...
if __name__ == "__main__":
    ...
    secret = os.environ.get('LMS_SECRET', None)
    if not secret:
        secret = str(input(f"What is the LMS Secret Key? "))
    ...
    elif lms_selection .upper()[0] == "B":
        lms = LMSBlackboard(url=url, api_key=creds, secret_key=secret, course_code=int(course_code))
