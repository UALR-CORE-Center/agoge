...
class CommunityBuildMirroring(BasePacketMirror):
    """Allow packet mirroring over shared-network builds (Community[Unit/Workout]).
    Standard builds will utilize SoloMirroring class instead.
    """
    ...
    def _load_build(self) -> dict:
        if not isinstance(self.build, dict):
            self.build = self.db.get(collection_name=DbCollections.UNIT, doc_id=self.build_id)
            if not self.build:
