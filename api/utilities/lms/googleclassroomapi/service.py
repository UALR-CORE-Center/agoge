...
class Service:
    ...
    def __init__(
        ...
        if not hasattr(self, 'initialized'):
            ...
            self.secret_name = 'classroom_api'
            self.secret_version = 'latest'
            self.resource_name = f"projects/{self.project}/secrets/{self.secret_name}/versions/{self.secret_version}"
            self.logger = Logger(LoggerNames.API, class_name=self.__class__.__name__)
            self.__credentials = None
            self.__service_account_info = None
            self.client = self.get_client()

    def _get_service_account_info(self) -> str:
        if not self.__service_account_info:
            secrets_client = secretmanager.SecretManagerServiceClient()
            response = secrets_client.access_secret_version(request={"name": self.resource_name})
    ...
    def _authenticate_from_file(self) -> None:
        """Authenticates user with API and sets credentials.

        This method authenticates the user with the Google API and sets the
        credentials for further API interactions. It raises exceptions if
        ...
        Raises:
            ...
            GoogleClassroomException: For other exceptions during authentication.
        """
        print('Loading credentials from service account file ...')
        if not self.service_account_path:
            GoogleClassroomException(message='Cannot load service account file: '
                                             'Invalid or missing value for `service_account_path`')
        try:
            if self.user:
