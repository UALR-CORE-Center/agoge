...
class Courses(GoogleClassroomObject):
    ...
    def get_teachers(
            self,
            course_id: str,
            known_teachers: dict = None
    ) -> dict:
        """Retrieves a dictionary of all teachers in a course

        Returns:
            dict: All teachers in a course
        """
        return Teacher(self._client).list(course_id=course_id, known_teachers=known_teachers)
    ...
    def get_all_teachers(self) -> dict:
        ...
        for course in courses:
            teachers.update(self.get_teachers(course['id'], teachers))
        return teachers
