...
from common.models.agoge import LMSConnectionModel, LMSQuizQuestionsModel, LMSQuizAnswerModel
from common.utilities.id_generator import IdGenerator
from ..lms.lms import (
    LMS,
    LMSSpec,
    LMSSpecIncompleteInstructorSettingsError,
    LMSUserNotFound,
    LMSExceptionWithHttpStatus
)
...
class LMSCanvas(LMS):
    def __init__(
        ...
        if not url or not api_key:
            ValueError('Invalid value for LMS connection. URL or API_KEY cannot be None')
        super().__init__(course_code, url, api_key)
    ...
    def get_courses(self) -> list[dict]:
        ...
        return [{'id': str(x.id), 'name': x.name} for x in courses]

    def mark_question_correct(
        self,
        quiz_id: str,
        student_email: str,
        added_points: int
...
class LMSSpecCanvas(LMSSpec):
    def __init__(
        ...
        super().__init__(instructor, build_spec, course_code, lms_type,
                         env_dict, due_at, time_limit, allowed_attempts)

    def _get_connection(self) -> Union[Dict, LMSConnectionModel]:
        try:
