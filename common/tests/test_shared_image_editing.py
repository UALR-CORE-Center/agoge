"""Shared image edits require admin acknowledgment; copies are independent."""
...
from common.exceptions import BadRequest, Conflict, Forbidden, NotFound, NotReady, OperationTimeout
...
TEACHER = AgogeUser(uid='teacher', email='instructor@example.edu', permissions={'instructor': True}, settings={})
ADMIN = AgogeUser(uid='admin', email='admin@example.edu', permissions={'admin': True}, settings={})
...
def test_shared_actions_reject_tampered_client_ownership_before_queueing(editing, action, image_exists):
    ...
    with pytest.raises(Forbidden, match='Only administrators'):
        editing.service.process_action_on_list(TEACHER, {
...
def test_direct_shared_settings_or_delete_are_rejected(editing, method):
    ...
    with pytest.raises(Forbidden if method == 'update' else BadRequest):
        if method == 'update':
...
@pytest.mark.parametrize('requester,allowed', [(TEACHER, False), (ADMIN, True), (None, False)])
@pytest.mark.parametrize('method', ['get', 'list_images', 'list_project_images'])
def test_shared_edit_permission_is_derived_from_requester(editing, requester, allowed, method):
    editing.records['shared-router']['can_edit_shared'] = not allowed
    if method == 'get':
        image = editing.service.get('shared-router', requester=requester)
    elif method == 'list_images':
        image = editing.service.list_images(requester=requester)[0]
    else:
        editing.service.db.query.side_effect = lambda collection_name: (
            list(editing.records.values()) if collection_name == DbCollections.IMAGE else []
        )
        image = editing.service.list_project_images(requester=requester)['custom'][0]
    assert image.can_edit_shared is allowed


@pytest.mark.parametrize('action', [PubSub.Actions.CHECK_OUT, PubSub.Actions.CHECK_IN,
                                  PubSub.Actions.START, PubSub.Actions.STOP, PubSub.Actions.CANCEL])
def test_admin_can_operate_shared_image_without_changing_its_destination(editing, action):
    original = deepcopy(editing.records)
    editing.service.process_action_on_list(ADMIN, {
        'action': str(action.value), 'images': ['shared-router'], 'shared_edit_confirmed': True,
    })
    message = editing.service.pubsub_manager.msg.call_args.kwargs
    assert message['shared_edit_authorized'] == 'true'
    assert message[PubSub.EventAttributes.USER] == ADMIN.email
    assert message[PubSub.EventAttributes.IMAGE_NAME] == 'shared-router'
    if action == PubSub.Actions.CHECK_OUT:
        editing.image_api.get.assert_called_once_with(resource='image-shared-router',
            project=SHARED_PROJECT, fallback_to_shared=False)
    assert editing.records == original


@pytest.mark.parametrize('action', [PubSub.Actions.CHECK_OUT, PubSub.Actions.CHECK_IN])
@pytest.mark.parametrize('confirmed', [None, False, 'true', 1])
def test_admin_shared_checkout_and_save_require_explicit_boolean_acknowledgment(editing, action, confirmed):
    with pytest.raises(BadRequest, match='affect multiple sites'):
        editing.service.process_action_on_list(ADMIN, {
            'action': str(action.value), 'images': ['shared-router'], 'shared_edit_confirmed': confirmed,
        })
    editing.service.pubsub_manager.msg.assert_not_called()
    editing.image_api.get.assert_not_called()


def test_teacher_cannot_promote_own_privileges_using_confirmation_fields(editing):
    with pytest.raises(Forbidden):
        editing.service.process_action_on_list(TEACHER, {
            'action': str(PubSub.Actions.CHECK_OUT.value), 'images': ['shared-router'],
            'shared_edit_confirmed': True, 'shared_edit_authorized': 'true', 'is_admin': True,
        })
    editing.service.pubsub_manager.msg.assert_not_called()


def test_local_action_never_inherits_shared_authorization_from_same_batch(editing):
    editing.records['local-router'] = {**SOURCE_RECORD, 'name': 'local-router', 'self_link': LOCAL_URL}
    editing.service.process_action_on_list(ADMIN, {
        'action': str(PubSub.Actions.STOP.value), 'images': ['shared-router', 'local-router'],
    })
    assert [call.kwargs['shared_edit_authorized'] for call in
            editing.service.pubsub_manager.msg.call_args_list] == ['true', 'false']


def test_shared_settings_admin_confirmation_preserves_authoritative_source(editing):
    with pytest.raises(BadRequest, match='affect multiple sites'):
        editing.service.update_image('shared-router', {'description': 'New description'}, requester=ADMIN)
    editing.service.db.update.assert_not_called()
    editing.service.update_image('shared-router', {
        'description': 'New description', 'shared_edit_confirmed': True,
        'self_link': LOCAL_URL, 'is_shared': False, 'source_project': PROJECT,
    }, requester=ADMIN)
    saved = editing.service.db.update.call_args.kwargs['data']
    assert saved['description'] == 'New description'
    assert saved['self_link'] == SOURCE_URL


@pytest.mark.parametrize('entrypoint', ['create', 'put'])
def test_build_endpoints_cannot_overwrite_shared_record(editing, entrypoint):
    editing.service.handler = PubSub.Handlers
    editing.service._check_out = Mock()
    original = deepcopy(editing.records)
    data = {'action': str(PubSub.Actions.BUILD.value), 'server_name': 'shared-router',
            'machine_type': 'e2-medium', 'description': 'attempted overwrite',
            'image_template': 'shared-router', 'os': 'linux'}
    with pytest.raises(Conflict):
        if entrypoint == 'create':
            editing.service.create_image_server(TEACHER, data)
        else:
            editing.service.process_action_on_image(TEACHER, data, server_name='shared-router')
    assert editing.records == original
    editing.document.create.assert_not_called()
    editing.service._check_out.assert_not_called()


@pytest.mark.parametrize('course_object', [PubSub.CourseObjects.TEMPLATE_SERVER, PubSub.CourseObjects.LAB_SERVER])
@pytest.mark.parametrize('action', [PubSub.Actions.SNAPSHOT, PubSub.Actions.RESTORE, PubSub.Actions.DELETE])
def test_shared_snapshots_cannot_bypass_image_warning_flow(editing, course_object, action):
    from api.core.compute.snapshot import ComputeSnapshot
    snapshots = object.__new__(ComputeSnapshot)
    snapshots.env = editing.service.env
    snapshots.db = editing.service.db
    snapshots.logger = Mock()
    snapshots.pubsub_keys = PubSub.EventAttributes
    snapshots.pubsub_manager = Mock()
    with pytest.raises(BadRequest, match='Manage snapshots on a local copy'):
        snapshots.process_server_action('shared-router', {
            'action': action.value, 'course_object': course_object.value,
            'shared_edit_authorized': 'true', 'shared_edit_confirmed': True,
        })
    snapshots.pubsub_manager.msg.assert_not_called()


@pytest.mark.parametrize('endpoint,method', [('/compute/images/', 'post'),
                                            ('/compute/images/shared-router/', 'patch')])
def test_shared_permission_errors_are_returned_as_forbidden(copy_route, endpoint, method):
    copy_route.app.dependency_overrides[copy_route.module.teacher_required] = lambda: TEACHER
    copy_route.backend.process_action_on_list.side_effect = Forbidden('Only administrators can edit shared images.')
    copy_route.backend.update_image.side_effect = Forbidden('Only administrators can edit shared images.')
    response = getattr(copy_route.client, method)(endpoint, json={
        'action': str(PubSub.Actions.CHECK_OUT.value), 'images': ['shared-router'],
        'shared_edit_confirmed': True,
    })
    assert response.status_code == 403
    assert 'Only administrators' in response.json()['detail']


def test_shared_nonadmin_responses_hide_connection_material_without_mutating_copy_source(editing):
    original = deepcopy(editing.records['shared-router'])
    restricted = editing.service.get('shared-router', requester=TEACHER)
    assert restricted.human_interaction == []
    assert restricted.dns_record is None
    assert restricted.startup_script is None
    admin = editing.service.get('shared-router', requester=ADMIN)
    assert admin.human_interaction[0].username == 'instructor'
    assert admin.dns_record == original['dns_record']
    assert editing.records['shared-router'] == original
    copied = editing.service.copy_shared_image('shared-router', 'local-router')
    assert copied.human_interaction[0].username == 'instructor'
